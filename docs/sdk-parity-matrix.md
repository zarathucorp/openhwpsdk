...
| Header/footer and page numbering | `inventory-only` / `package-writer` / `COM-smoke` | `list-header-footer`, `set-header-footer-text`, `set-header-footer-apply-page-type`, and `page-number-set` cover focused cases. | Inventory before/after plus `validate-layout`; `page-number-set` also needs COM smoke. | Header/footer creation and rich header/footer authoring are not general-purpose yet. |
