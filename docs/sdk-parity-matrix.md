...
| Header/footer and page numbering | `inventory-only` / `package-writer` / `COM-smoke` | `list-header-footer`, `set-header-footer-text`, `set-header-footer-apply-page-type`, `add-header-footer-reference`, `add-header-footer-text`, and `page-number-set` cover focused cases. | Inventory before/after plus `validate-layout`; `page-number-set` also needs COM smoke. | `add-header-footer-reference` reuses an existing `idRef`; `add-header-footer-text` creates only a minimal one-paragraph text body with a section-unique header/footer `idRef`. Rich header/footer authoring is not general-purpose yet. |
