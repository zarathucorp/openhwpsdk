...
- 원본 문서 첫 표 덤프: [table0-dump.txt](/C:/Users/ZARATHU11/codex/openhwpsdk/artifacts/table0-dump.txt)
- 현재 표 선택 로직: [HwpSession.cs](/C:/Users/ZARATHU11/codex/openhwpsdk/src/OpenHwp.Automation/HwpSession.cs)
- 현재 CLI 진입점: [Program.cs](/C:/Users/ZARATHU11/codex/openhwpsdk/src/OpenHwp.Automation.Cli/Program.cs)
...
이 단계가 끝나면 수동으로 `test/1` 표 맵을 만들 수 있다.
...
## `test/1`에 바로 필요한 매핑 전략

`test/1`은 다음 순서로 적용하는 것이 맞다.
...
이 순서를 지키면 `test/1` 같은 양식 문서도 전역 치환에 덜 의존하고 안정적으로 수정할 수 있다.
