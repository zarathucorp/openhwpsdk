...
`table-cell-align-package` applies horizontal paragraph alignment and/or cell vertical alignment to one cell or a rectangular range. Horizontal alignment creates a cloned `paraPr` in `Contents/header.xml` instead of mutating the shared source style, then retargets affected direct cell paragraphs; vertical alignment updates `hp:subList@vertAlign`. Validate with direct XML inspection of both the selected `Contents/sectionN.xml` and `Contents/header.xml`.
`table-cell-background-package` adds, changes, or clears a solid background by cloning each affected cell's current `borderFill`, updating `hc:fillBrush/hc:winBrush@faceColor`, and retargeting only the selected cells. `--color none` removes the cloned `fillBrush`. The command preserves existing borders and applies to merged cells through span intersection, so validate the target cell `borderFillIDRef` in the selected `Contents/sectionN.xml` and the cloned brush in `Contents/header.xml`.
`table-cell-diagonal-package` adds, changes, or clears cell diagonal lines by cloning the affected cells' current `borderFill` definitions, updating `hh:slash` and `hh:backSlash`, and retargeting only the selected cells. `--direction slash|backslash|both|none` maps to `CENTER` or `NONE`, while `--width` and `--color` override the cloned `hh:diagonal` values. Without `--base-border-fill-id`, each affected cell keeps its own existing border/fill defaults. Validate with direct XML inspection of the selected `Contents/sectionN.xml` cell `borderFillIDRef` values and `Contents/header.xml` borderFill entries.
`table-cell-size-package` equalizes column widths and/or row heights in the safe simple-table subset. `--equalize-widths` keeps the current total table width and redistributes first-row `hp:cellSz@width` values across all columns; `--equalize-heights` keeps the current total table height and redistributes row max `hp:cellSz@height` values across all rows. It rejects merged, nested, sparse, or irregular tables. Validate with direct XML inspection and `validate-layout` because content does not change.
