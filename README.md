...
For the supported startup R&D submission profile, use `fill-submission-template ... --profile r-and-d-startup-2026 --report report.md`. Add `--asset-root <dir>` when Markdown image paths should also be resolved under a separate image root. Body Markdown tables default to `--markdown-table-mode text` so the original HWPX table count stays stable; use `--markdown-table-mode render` only when new HWPX table objects are acceptable. The fill report includes template/profile compatibility, image path candidates, mapped/unmapped image counts, and the sibling layout report classifies issues as `expected-change`, `review-needed`, or `blocking`. `validate-layout` also accepts `--allow-table-row-change <indexes>` and `--max-leading-style-drift <count>`.
