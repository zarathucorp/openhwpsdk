...
Use `table-column-package` for COM-free column add/delete on the same safe text-cell table subset in the selected body section. It changes table width by adding/removing cloned cell widths, so validate intentional changes with `validate-layout --allow-table-column-change <index>` using the validator's all-table index. For add, `--column` means insert after that zero-based column; omit it to append after the last column. For delete, `--column` is the zero-based first column to delete.
...
Use `table-cell-background-package` to add, change, or clear a solid cell background without mutating shared `borderFill` definitions in place. It clones each affected cell's current `borderFill`, updates or removes `hc:fillBrush/hc:winBrush`, and retargets the selected cells. Use `--color none` to clear the fill brush.
...
Use `table-cell-size-package` to equalize column widths and/or row heights in a simple unmerged top-level table. It preserves total table width/height, redistributes the existing `hp:cellSz` values across all columns or rows, rejects merged/nested/irregular tables, and should be verified by direct XML inspection plus `validate-layout`.
