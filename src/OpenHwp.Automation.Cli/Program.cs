    ...
    internal static class Program
    {
        ...
        private static int Run(string[] args)
        {
            ...
            switch (command)
            {
                ...
                case "add-header-footer-text":
                    return AddHeaderFooterText(commandArgs);
        ...
        private static int AddHeaderFooterText(string[] args)
        {
            if (args.Length < 3)
            {
                Console.Error.WriteLine("Usage: add-header-footer-text <inputHwpxPath> <outputHwpxPath> --kind header|footer --section sectionName --id-ref idRef --apply-page-type BOTH|EVEN|ODD --text text [--report reportMarkdownPath]");
                return 1;
            }
            ...
            var result = HwpxHeaderFooterBodyWriter.AddTextBody(args[1], args[2], kind, section, idRef, applyPageType, text, reportPath);
            Console.WriteLine("added=" + BoolText(result.Added));
            Console.WriteLine("existing_bodies=" + result.ExistingBodies.ToString(CultureInfo.InvariantCulture));
            Console.WriteLine("existing_references=" + result.ExistingReferences.ToString(CultureInfo.InvariantCulture));
            Console.WriteLine("id_ref_conflicts=" + result.IdRefConflicts.ToString(CultureInfo.InvariantCulture));
            Console.WriteLine("apply_page_type_conflicts=" + result.ApplyPageTypeConflicts.ToString(CultureInfo.InvariantCulture));
            Console.WriteLine("added_bodies=" + result.AddedBodies.ToString(CultureInfo.InvariantCulture));
            Console.WriteLine("added_references=" + result.AddedReferences.ToString(CultureInfo.InvariantCulture));
            Console.WriteLine("apply_page_type=" + result.ApplyPageType);
            if (!string.IsNullOrWhiteSpace(result.PartPath))
            {
        ...
        private static void PrintUsage()
        {
            ...
            Console.WriteLine("  add-header-footer-text <inputHwpxPath> <outputHwpxPath> --kind header|footer --section sectionName --id-ref idRef --apply-page-type BOTH|EVEN|ODD --text text [--report reportMarkdownPath]");
